...
<div class="container" id="kanban-page">
  ...
  <form method="GET">
    <select name="pid" onchange="this.form.submit()">
      ...
      <?php if (count($owned_products) === 0): ?>
        <option value="" disabled>(אין מוצרים זמינים)</option>
      <?php else: ?>
...
<div id="kanban-container">
  ...
  <p><a href="product_dashboard.php" class="register-link" style="margin-top:30px;">⬅ חזרה לעמוד הראשי</a> </p>
...
// Delete handler, attached inline for reliability
function onDeleteTaskClick(e) {
  const btn = e.currentTarget;
  const tid = btn.getAttribute("data-id") || "";
  if (!tid) return;
  const task = document.getElementById("task-" + String(tid));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Enable drag for existing tasks
  ...
  document.getElementById("modalForm").addEventListener("submit", e => {
    ...
    // Avoid template literals inside querySelector to prevent escaping bugs
